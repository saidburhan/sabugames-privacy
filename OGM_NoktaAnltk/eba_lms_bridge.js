...
(function () {
 ...
 function getEbaLmsWindow() {
  // 1) Önce mainFrame içinden bak (senin sistemde burada)
  try {
    const mf = window.top?.document?.getElementById("mainFrame");
    const doc = mf?.contentWindow?.document;
    const frm = doc?.getElementById("frmSkillBasedApp");
    if (frm?.contentWindow) return frm.contentWindow;
  } catch (e) {}

  // 2) Yedek: eski yöntem (bazı sayfalarda işe yarar)
  try {
    const frm = window.parent?.document?.getElementById("frmSkillBasedApp");
    if (frm?.contentWindow) return frm.contentWindow;
  } catch (e) {}

  // 3) Son çare: top'ta arama
  try {
    const frm = window.top?.document?.getElementById("frmSkillBasedApp");
    if (frm?.contentWindow) return frm.contentWindow;
  } catch (e) {}
