...
(function () {
 ...
 function getEbaLmsWindow() {
  ...
  try {
    ...
    const frm = doc?.getElementById("frmSubjectApp");
    if (frm?.contentWindow) return frm.contentWindow;
  } catch (e) {}

  // 2) Yedek: eski yöntem (bazı sayfalarda işe yarar)
  try {
    const frm = window.parent?.document?.getElementById("frmSubjectApp");
    if (frm?.contentWindow) return frm.contentWindow;
  } catch (e) {}

  // 3) Son çare: top'ta arama
  try {
    const frm = window.top?.document?.getElementById("frmSubjectApp");
    if (frm?.contentWindow) return frm.contentWindow;
  } catch (e) {}
